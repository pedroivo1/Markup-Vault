function optimize_mp4
    ...
    find "$ROOT_PATH" -type f -name "* - o.mp4" -print0 | sort -zV | while read -lz video_file
        ...
        # --- FPS and Dynamic GOP Logic (Read-Only) ---
        ...
        if test -n "$raw_fps"
            ...
            # Calculate the GOP based on the file's original FPS
            set target_gop (awk -v fps="$fps" 'BEGIN { printf "%.0f", fps * 2 }')

            echo " └─> Original FPS Maintained: $fps"
            echo " └─> Keyframes (GOP): Every $target_gop frames (2s)"
        else
            # Error prevention in case the file has no FPS metadata
            set target_gop 60
            echo " └─> Warning: FPS not detected. GOP fixed at 60 frames."
        end
        ...
        echo " └─> FFMPEG Command:"
        echo "     ffmpeg -nostdin -hide_banner -loglevel warning -stats -n -vaapi_device /dev/dri/renderD128 -i \"$video_file\" -vf \"format=nv12,hwupload\" -c:v hevc_vaapi -qp \"$target_qp\" -g \"$target_gop\" -c:a aac -b:a 64k \"$output_file\""
        ...
        ffmpeg -nostdin -hide_banner -loglevel warning -stats -n \
               ...
               -c:v hevc_vaapi -qp "$target_qp" -g "$target_gop" \
               -c:a aac -b:a 64k \
               "$output_file"
